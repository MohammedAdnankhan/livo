...
exports.createTenantUser = async (req, res, next) => {
  ...
  try {
    ...
    const adminEmailMatch = await Tenant.findOne({
    ...
    if (adminEmailMatch) {
      res.status(400).json({
        success: false,
        code: 400,
        message: "Email already used as tenant admin email",
      });
      return next();
    }
    // 2. Check if user with same email and tenant_id already exists
    const userEmailMatch = await TenantUser.findOne({
    ...
    if (!password) {
      res
        .status(400)
        .json({ success: false, code: 400, message: "Password is required" });
      return next();
    }
    ...
    if (tenant.user_add_limit !== null && userCount >= tenant.user_add_limit) {
      res.status(400).json({
        success: false,
        code: 400,
        message: "User add limit reached for this tenant",
      });
      return next();
    }
...
exports.getAllCategory = async (req, res, next) => {
  try {
    return res.status(200).json({
      success: true,
      code: 200,
      message: "Tenants Category fetched",
...
exports.getTenantUserById = async (req, res, next) => {
  try {
    ...
    if (!user) {
      res
        .status(404)
        .json({ success: false, code: 404, message: "Tenant user not found" });
      return next();
    }
    res.status(200).json({
      success: true,
...
exports.updateTenantUser = async (req, res, next) => {
  try {
    ...
    const [updated] = await TenantUser.update(updateData, {
      where: { user_id },
    });
    if (!updated) {
      res
