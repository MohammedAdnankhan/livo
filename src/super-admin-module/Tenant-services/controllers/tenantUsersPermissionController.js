...
exports.createPermission = async (req, res, next) => {
  ...
  try {
    ...
    const permission = await TenantUsersPermission.create({
      role_name,
      tenant_id,
...
exports.getAllPermissions = async (req, res, next) => {
  try {
    ...
    if (!tenant_id) {
      return res
        .status(400)
        .json({ success: false, code: 400, message: "tenant_id is required" });
    }
    const permissions = await TenantUsersPermission.findAll({
      where: { tenant_id: tenant.connected_admin_id },
      order: [["role_name", "ASC"]],
    });
    res.status(200).json({
      success: true,
...
exports.getPermissionById = async (req, res, next) => {
  try {
    ...
    const permission = await TenantUsersPermission.findOne({
    ...
    if (!permission) {
      return res.status(404).json({
        success: false,
        code: 404,
        message: "Permission not found for this tenant",
      });
    }
    res.status(200).json({
      success: true,
...
exports.updatePermission = async (req, res, next) => {
  try {
    ...
    await TenantUsersPermission.update(updateData, {
      where: { role_id, tenant_id },
    });
    const updatedPermission = await TenantUsersPermission.findOne({
      where: { role_id, tenant_id },
    });
    res.status(200).json({
      success: true,
...
exports.deletePermission = async (req, res, next) => {
  try {
    ...
    const deleted = await TenantUsersPermission.destroy({
      where: { role_id, tenant_id },
    });
    if (!deleted) {
      return res.status(404).json({
